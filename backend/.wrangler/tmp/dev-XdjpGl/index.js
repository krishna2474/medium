...
// .wrangler/tmp/bundle-1kbmsS/checked-fetch.js
...
// node_modules/@krishnakukreja85/medium-common/dist/index.js
var require_dist = __commonJS({
  "node_modules/@krishnakukreja85/medium-common/dist/index.js"(exports) {
    "use strict";
    init_checked_fetch();
    init_modules_watch_stub();
    init_process();
...
// .wrangler/tmp/bundle-1kbmsS/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();
init_process();
init_buffer();

// .wrangler/tmp/bundle-1kbmsS/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
init_process();
init_buffer();
...
userRouter.post("/signup", async (c) => {
  ...
  try {
    ...
    const { success } = import_medium_common.signupInputs.safeParse(body);
    if (!success) {
      c.status(411);
      return c.json({
    ...
    const token = await sign2({ id: resp.id }, c.env.JWT_SECRET);
...
blogRouter.use("/*", async (c, next) => {
  ...
  const isValid = await verify2(token, c.env.JWT_SECRET);
  if (isValid) {
...
blogRouter.post("/", async (c) => {
  ...
  try {
    const { success } = import_medium_common2.createBlogInputs.safeParse(body);
    if (!success) {
      c.status(411);
      return c.json({
...
blogRouter.put("/", async (c) => {
  ...
  try {
    const { success } = import_medium_common2.updateBlogInputs.safeParse(body);
    if (!success) {
      c.status(411);
      return c.json({
        message: "Incorrect Inputs"
